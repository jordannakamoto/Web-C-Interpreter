...
export default function RF() {
    ...
    // More Accurate Split
    // function customSplit(input) {
    //     // Matches single or double quotes, the contents between them, non-whitespace sequences, or specific characters
    //     const regex = /(".*?"|'.*?'|[^\s|()]+)/g;
    //     const tokens = input.match(regex) || [];
    
    //     const processedTokens = tokens.flatMap(token => {
    //         // Check if the token is a quoted string for special string processing
    //         if ((token.startsWith(`"`) && token.endsWith(`"`)) || (token.startsWith(`'`) && token.endsWith(`'`))) {
    //             // Return opening quote, the content, and closing quote as separate tokens
    //             return [token.charAt(0), token.substring(1, token.length - 1), token.charAt(token.length - 1)];
    //         }
    ...
    useEffect(() => {
        const cstData = programOutput['astData'];
